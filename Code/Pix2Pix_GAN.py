...
Original file is located at
    https://colab.research.google.com/drive/1-bTBkLTcMm-t9A622ptEiC8t8I4lKioy
"""
...
#创建images和saved_model两个文件夹，下属 test1 文件夹
...
class ImageDataset_color(Dataset):
  ...
  def __len__(self):
    return len(self.files)#把处理好的图片存到指定路径下   
class ImageDataset_edge(Dataset):
  ...
  def __getitem__(self,index):
    img_A = Image.fromarray(np.array(cv2.cvtColor(cv2.imread(self.files[index % len(self.files)]),cv2.COLOR_BGR2RGB)),"RGB")
    img_B = Image.fromarray(np.array(cv2.cvtColor(cv2.Canny(cv2.GaussianBlur(cv2.cvtColor(cv2.imread(self.files[index%len(self.files)]), cv2.COLOR_BGR2GRAY),(5, 5), 0),50,150),cv2.COLOR_GRAY2RGB)),"RGB")
    img_A = self.transform(img_A)
    img_B = self.transform(img_B)
    return {"A": img_A, "B": img_B}
  def __len__(self):
...
criterion_pixelwise = torch.nn.L1Loss()#这一步不知道是啥，盲猜正则化的loss
# Loss weight of L1 pixel-wise loss between translated image and real image
lambda_pixel = 100 #应该是同上一步相关
...
#如果epoch不为0，则直接加载G,D分别的模型，否则初始化
if epoch != 0:
...
for epoch in range(epoch,n_epochs):
  ...
  if opt.checkpoint_interval != -1 and epoch % checkpoint_interval==0:
